		...
		while(i<supplyStacks.size()) {
			...
			while(j<supplyStacks.get(i).size()) {
				if(supplyStacks.get(i).get(j).getName().equals(cardName)) {
					...
					if(supplyStacks.get(i).size()==0){
						supplyStacks.remove(i);
					}
					return c;
